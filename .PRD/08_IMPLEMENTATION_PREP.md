...
| 블록 | 핵심 사실 | 쓰는 곳 |
|---|---|---|
| hooks (`hooks/hooks.json`) | `deny`=하드 차단 / `ask`=권한모드 의존(자동승인선 통과) | Harness(완성)·Loop |
...
- **Harness** 🛟 — 빌딩블록(hooks) **완성**(자가테스트 32/0). 남은 것 = **실초보 베타**.
...
근거: 데이터상 Harness만 동작코드(47파일)·나머지 3종은 PRD만(8~9파일). "한 번에 다 만들면 품질↓"(03)·"미검증 위에 쌓기 금지"·"시뮬0·베타0=80% 실패".
